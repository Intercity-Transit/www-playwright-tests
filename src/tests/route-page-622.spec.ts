...
test.describe('Tests for /plan-your-trip/routes/622 and route times @routes', () => {
  ...
  test('Route 622 schedule validation @routes', async ({ page }) => {
    ...
    await expect
      .soft(timesTable, 'Route should include Pacific Ave at Rockcress Dr as a stop')
      .toContainText('Pacific Ave at Rockcress Dr');
    await expect.soft(timesTable, 'Route should include 7:00 a.m. departure time').toContainText('7:00 a.m.');
    await expect.soft(timesTable, 'Route should include 12:12 a.m. departure time').toContainText('12:12 a.m.');
  });
});
